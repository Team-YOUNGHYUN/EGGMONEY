  ...
  <div class="container">
    ...
    <fieldset class="required-input">
      ...
      <input
        ...
        class="view"
      />
      <div class="msg" id="none" v-if="getLoginUser.password === '' || confirmPassword === ''"/> 
      <div class="msg" id="true" v-else-if="getLoginUser.password === confirmPassword">일치합니다.</div>
      ...
      <div id="gender" v-if="getLoginUser.gender='male'">
        <input type="radio" id="male" value="male" class="view" disabled checked/>
        <label for="male">남성</label>
        <input type="radio" id="female" value="female" class="view" style="margin-left: 7px" disabled/>
        <label for="female">여성</label>
      </div>
      <div id="gender" v-else>
        <input type="radio" id="male" value="male" class="view" disabled/>
        <label for="male">남성</label>
        <input type="radio" id="female" value="female" class="view" style="margin-left: 7px" disabled checked/>
        <label for="female">여성</label>
      </div>
      <br />
    </fieldset>
    ...
    <fieldset class="optional">
      ...
      <input type="number" id="height" v-model="getLoginUser.height" class="view" /><br />
      <label for="weight">몸무게</label>
      <input type="number" id="weight" v-model="getLoginUser.weight" class="view" /><br />
      <label for="bodyFat">체지방률</label>
      <input type="number" id="bodyFat" v-model="getLoginUser.bodyFat" class="view" /><br />
      <label for="muscleMass">골격근량</label>
      <input type="number" id="muscleMass" v-model="getLoginUser.muscleMass" class="view" /><br />
      <button id="button" class="btn btn-success" @click="updateUser">등록</button>
    </fieldset>
  </div>
</template>
...
export default {
    ...
    methods:{
        async updateUser() {
            ...
            if (!this.getIsUnqNickname && this.originalNickname !== this.getLoginUser.nickname) {
         ...
         checkNickname() {
            ...
            this.$store.dispatch("checkNickname", this.getLoginUser.nickname)
            .then(() => {
